...
$outKeyFile = [System.IO.Path]::Combine($outputPath, "cakey.pem")
$outKeyFile2 = [System.IO.Path]::Combine($outputPath, "ca.key")
...
try {
	...
	& $openSSL  req -x509 -newkey rsa -days 7300 -config $targetConfig -out $outFile -passout file:capwd
	& $openSSL  ca -create_serial -config $targetConfig -gencrl -out $outCrlFile -passin file:capwd
	
	& $openSSL  rsa -in $outKeyFile -out $outKeyFile2 -passin file:capwd
	& $openSSL  pkcs12 -export -in $outFile -inkey $outKeyFile2 -CSP $csp -keyex -out $resultFile1 -passout file:capwd
