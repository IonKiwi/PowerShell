...
database = `$dir/out/index.txt
new_certs_dir = `$dir/out
private_key = `$dir/out/imendkey.pem
serial = `$dir/out/serial
...
$outKeyFile1 = [System.IO.Path]::Combine($outputPath, "imendkey.pem")
...
try {
	...
	& $openSSL req -new -config $targetConfig -key $outKeyFile1 -out $outRequestFile -passin file:imendpwd
