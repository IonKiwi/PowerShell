...
database = `$dir/out/index.txt
new_certs_dir = `$dir/out
private_key = `$dir/out/certkey.pem
serial = `$dir/out/serial
...
$outKeyFile1 = [System.IO.Path]::Combine($outputPath, "certkey.pem")
...
try {
	...
	if ($isServer -or $isClient) {
		...
		& $openSSL  req -new -config $targetConfig -key $outKeyFile1 -out $outRequestFile -passin file:certpwd
